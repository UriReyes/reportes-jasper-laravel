...
trait ApiSite24x7
{
    public function getRefreshToken()
    {
        ...
        $response = Http::connectTimeout(3)->timeout(1200)->asForm()->retry(3, 900)->post("{$url}/token", data: [
    ...
    public function getCustomers($url, $refresh_token)
    {
        ...
        $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
            'Content-Type' => 'application/json;charset=UTF-8',
            'Accept' => 'application/json; version=2.0',
            'Authorization' => $authorization
        ])->retry(3, 5000)->get("{$url}/short/msp/customers");
    ...
    public function getMonitorGroups($url, $zaaid, $refresh_token, $monitor_group)
    {
        ...
        $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
            ...
            'Authorization' => $authorization,
            'Cookie' => $cookie
        ])->retry(3, 5000)->get("{$url}/monitor_groups/{$monitor_group}");
        $response = $response->json();
        return $response['data'];
    }
    ...
    public function getMonitor($url, $zaaid, $refresh_token, $monitor_id)
    {
        ...
        $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
            ...
            'Authorization' => $authorization,
            'Cookie' => $cookie
        ])->retry(3, 5000)->get("{$url}/monitors/{$monitor_id}");
        $response = $response->json();
        return $response['data'];
    }
    ...
    public function getCurrentStatus($url, $zaaid, $refresh_token, $monitor_id)
    {
        try{
            ...
            $response = Http::connectTimeout(5)->withHeaders([
                ...
                'Authorization' => $authorization,
                'Cookie' => $cookie
            ])->timeout(5000)->retry(5, 5000)->get("{$url}/current_status/{$monitor_id}");
            $response = $response->json();
            return $response['data'];
        } catch (\Exception $e) {
            return [
    ...
    public function getAvailabilityReport($url, $monitor_id, $zaaid, $refresh_token, $atributos)
    {
        ...
        $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
            ...
            'Authorization' => $authorization,
            'Cookie' => $cookie
        ])->retry(3, 5000)->get("{$url}/reports/availability_summary/${monitor_id}${atributos}");
        $response = $response->json();
        return $response;
    }
    ...
    public function getPerformance($url, $monitor_id, $zaaid, $refresh_token, $attributos)
    {
        try{   
            ...
            $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
                ...
                'Authorization' => $authorization,
                'Cookie' => $cookie
            ])->retry(3, 5000)->get("{$url}/reports/performance/{$monitor_id}{$attributos}");
            $response = $response->json();
            return $response;
        } catch (\Exception $e) {
            return [
    ...
    public function getPerformanceCharts($url, $monitor_id, $zaaid, $refresh_token, $attributos)
    {
        try{   
            ...
            $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
                ...
                'Authorization' => $authorization,
                'Cookie' => $cookie
            ])->retry(3, 5000)->get("{$url}/charts/performance/{$monitor_id}{$attributos}");
            $response = $response->json();
            return $response;
        } catch (\Exception $e) {
            return [
    ...
    public function getPerformanceDiskWidget($url, $monitor_id, $zaaid, $refresh_token, $attributos)
    {
        try{
            ...
            $response = Http::connectTimeout(3)->timeout(5000)->withHeaders([
                ...
                'Authorization' => $authorization,
                'Cookie' => $cookie
            ])->retry(3, 5000)->get("{$url}/monitors/widget_details/{$monitor_id}{$attributos}");
            $response = $response->json();
            return $response;
        } catch (\Exception $e) {
            return [
